...
import { toggleStyles } from "@/components/client/navigation/menuToggle";
...
export function handleClick(event: React.MouseEvent, menu_items: MenuItems) {
	...
	if (
		...
		targetElement.parentElement.classList.contains("pure-menu-item")
	) {
...
export function handleScroll(menu_items: MenuItems) {
	const elem = document.scrollingElement || document.documentElement,
		...
		elements = Array.prototype.slice
			.call(document.body.querySelectorAll("section.menu-block"))
			.reverse();
	...
	for (let i = 0; i < elements.length; i++) {
		if (
			...
			menu_items.current?.forEach((item) => {
				if (
					item
						.querySelector(".pure-menu-link")
						?.getAttribute("href")
						?.replace(/.*#/, "") === elements[i].id
				) {
