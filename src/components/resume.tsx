...
import {
	...
	TypeResume,
	isTypePersonalExperience,
	isTypeProfessionalExperience,
...
export default async function renderResume(
	resume: TypeResume<"WITHOUT_UNRESOLVABLE_LINKS", string>
) {
	const professionalExperience: React.ReactNode[] = [],
...
export async function renderExperience(
	experience:
		| TypePersonalExperience<"WITHOUT_UNRESOLVABLE_LINKS", string>
		| TypeProfessionalExperience<"WITHOUT_UNRESOLVABLE_LINKS", string>
) {
	if (isTypeProfessionalExperience(experience)) {
		return (
			<ProfessionalExperience
				key={experience.sys.id}
				company={experience.fields.companyName}
		...
		return (
			<PersonalExperience
				...
				shortDescription={experience.fields.shortDescription}
				description={experience.fields.description}
			/>
		);
	}
