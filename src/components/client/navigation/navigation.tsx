...
export default function Navigation( { items, slug, ...props }: NavigationProps & JSX.IntrinsicElements["nav"] ) {
	...
	return <nav {...props}>
		<ul className="pure-menu-list">
			...
			{items.home.fields.content.map(section => section && isTypePageSection(section) &&
				<li key={section.sys.id} className="pure-menu-item">
					<a className={`pure-menu-link ${styles.link}`} href={`/#${section.fields.slug}`}>
		...
		{items.pages.length > 0 && <ul className="pure-menu-list">
			<li className="pure-menu-separator"></li>
			{items.pages.map(page => <li key={page.sys.id} className={`pure-menu-item ${slug == page.fields.slug? `pure-menu-active ${styles.active}` : ""}`}>
